...
DNNSetup::DNNSetup(Context & ctx, const double & zero_padding_value) {
  ...
  */
  ...
  m_inputs_template_ttag.push_back(DNNInput("lepton_ptrel_nextjet", "Rel. p_{T}(lepton, nearest AK4 jet)", 0, 500));
  ...
  */
  ...
  m_inputs_template_wtag.push_back(DNNInput("lepton_ptrel_nextjet", "Rel. p_{T}(lepton, nearest AK4 jet)", 0, 500));
...
void DNNSetup::calculate_inputs_for_ttag_dnn(Event & event) {
  ...
  */
  ...
  if(no_xjets) {
    for(int j = 0; j < 6; j++) values.at(i++) = m_zeropadding;
  } else {
    auto nxj = nextJet(topjet, xjets);
    values.at(i++) = deltaR(topjet.v4(), *nxj);
    values.at(i++) = (*nxj).v4().M();
    values.at(i++) = (*nxj).v4().Pt();
    values.at(i++) = (*nxj).v4().Eta();
    values.at(i++) = (*nxj).v4().Phi();
    values.at(i++) = (*nxj).btag_DeepJet();
  }
...
void DNNSetup::calculate_inputs_for_wtag_dnn(Event & event) {
  ...
  */
  ...
  if(no_xjets) {
    ...
    auto nxj = nextJet(wjet, xjets);
    values.at(i++) = deltaR(wjet.v4(), *nxj);
    values.at(i++) = (*nxj).v4().M();
    values.at(i++) = (*nxj).v4().Pt();
    values.at(i++) = (*nxj).v4().Eta();
    values.at(i++) = (*nxj).v4().Phi();
